...
# Filename:     plot4.R                                                        #
#                                                                              #
# Description:  File containing the R language code that produces the fourth   #
...
#-------------------------------------------------------------------------------
#Step 4:
#Finally, we proceed to the actual realization of the plot. It is a multiple
#base plot that will be stored in a png file called "plot4.png":

#We open a PNG device and we configure it with the dimensions requested:
png("plot4.png", width = 480, height = 480)

#We use the "par()" function to specify a global graphics parameter. Now,
#subsequent figures will be drawn in a 2 by 2 array (by columns):
par(mfcol = c(2, 2))

        #1. Graphical representation of the "Global_active_power" variable:
        #We create the desired plot:
        plot(as.numeric(as.character(interval$Global_active_power)),
             type = "l", axes = FALSE, ann = FALSE, col = "black")
        ...
        #2. Simultaneous representation of the three "Sub_metering" variables:
        #We begin the plot representing the first variable:
        plot(as.numeric(as.character(interval$Sub_metering_1)), type = "l",
                axes = FALSE, ann = FALSE, col = "black")
        
        #We add the data of the other two variables:
        lines(as.numeric(as.character(interval$Sub_metering_2)), col = "red")
        lines(as.numeric(as.character(interval$Sub_metering_3)), col = "blue")

        #We finish the plot with its annotation:
        ...
        #3. Graphical representation of the "Voltage" variable:
        #We create the desired plot:
        plot(as.numeric(as.character(interval$Voltage)), xlab = "datetime",
                ylab = "Voltage", type = "l", axes = FALSE, col = "black")
        ...
        #4. Graphical representation of the "Global_reactive_power" variable:
        #We start the required plot:
        plot(as.numeric(as.character(interval$Global_reactive_power)),
                xlab = "datetime", ylab = "Global_reactive_power", type = "l",
                axes = FALSE, col = "black")
        
        #We continue with its annotation:
        box()
        axis(1, at=c(1, 1440, 2880), lab=c("Thu","Fri", "Sat"))
